...
在 `src/data/lunar-practice.json` 中维护安排：`time` 是适用于初一、十五法会的开始时间，目前为 `null`（待公布）；确认后可填 `"10:00"` 等费城当地时间。`timeOverrides` 可为某日单独设置，例如 `{"2026-09-25":"11:00"}`；某日填 `null` 会覆盖默认时间，显示待公布。`excludedDates` 可加入停办法会的日期，例如 `["2026-09-11"]`。以上均为维护格式示例，当前尚未设定这些时间或停办日期。初一、十五法会的时间不自动沿用周日或地藏法会时间。
...
- 每月农历初一、十五举行法会，用户于 2026-09-07 确认；开始时间尚待公布。
